...
SCREENSIZE = (640, 640)
PICTURE_ROOT_DIR = os.path.join(os.getcwd(), 'Puzzle/resources/pictures')
FONTPATH = os.path.join(os.getcwd(), 'Puzzle/resources/font/PressStart2P.TTF')
...
def ShowEndInterface(screen, width, height):
    ...
    font1 = pygame.font.Font(FONTPATH, width//24)
    font2 = pygame.font.Font(FONTPATH, width//34)
    title = font1.render('You Solved The Puzzle!', True, RED)
    content = font2.render('Press Any key to play again', True, BLUE)
    rect1 = title.get_rect()
    rect2 = content.get_rect()
    rect1.midtop = (width/2, height/2.5)
...
def ShowStartInterface(screen, width, height):
    screen.fill(BACKGROUNDCOLOR)
    tfont = pygame.font.Font(FONTPATH, width//8)
    cfont = pygame.font.Font(FONTPATH, width//36)

    title = tfont.render('Puzzle', True, RED)
    content1 = cfont.render(
        'Press H, M or L to choose a puzzle', True, BLUE)
